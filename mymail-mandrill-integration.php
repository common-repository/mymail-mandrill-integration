...
Plugin URI: https://evp.to/mymail?utm_campaign=wporg&utm_source=MyMail+Mandrill+Integration
...
define( 'MYMAIL_MANDRILL_VERSION', '0.4.1' );
...
class MyMailMandrill {
		...
		if ( function_exists( 'mailster' ) ) {
			...
			add_action(
				...
				function() {
					...
					$name = 'MyMail Mandrill Integration';
					$slug = 'mailster-mandrill/mailster-mandrill.php';
	...
	public function initsend( $mailobject ) {
		...
		( ! defined( 'MYMAIL_DOING_CRON' ) && mymail_option( MYMAIL_MANDRILL_ID . '_backlog' ))
			? mymail_notice( sprintf( __( 'You have %1$s mails in your Backlog! %1$s', 'mymail-mandrill' ), '<strong>' . mymail_option( MYMAIL_MANDRILL_ID . '_backlog' ) . '</strong>', '<a href="http://eepurl.com/rvxGP" class="external">' . __( 'What is this?', 'mymail-mandrill' ) . '</a>' ), 'error', true, 'mandrill_backlog' )
	...
	public function presend( $mailobject ) {
		...
		if ( $track = mymail_option( MYMAIL_MANDRILL_ID . '_track' ) ) {
			$mailobject->mailer->addCustomHeader( 'X-MC-Track', $track );
		}
		if ( $subaccount = mymail_option( MYMAIL_MANDRILL_ID . '_subaccount' ) ) {
	...
	public function deliverytab() {
		...
		wp_enqueue_script( 'mymail-mandrill-settings-script', $this->plugin_url . '/js/script.js', array( 'jquery' ), MYMAIL_MANDRILL_VERSION );
		...
		<table class="form-table">
			...
			<tr valign="top">
				<th scope="row">&nbsp;</th>
				<td><p class="description"><?php echo sprintf( __( 'You need a %s to use this service!', 'mymail-mandrill' ), '<a href="https://mandrill.com/signup/" class="external">Mandrill Account</a>' ); ?></p>
				</td>
			</tr>
			<?php endif; ?>
			<tr valign="top">
				<th scope="row"><?php _e( 'Mandrill Username', 'mymail-mandrill' ); ?></th>
			...
			<tr valign="top">
				<th scope="row"><?php _e( 'Mandrill API Key', 'mymail-mandrill' ); ?></th>
				<td><input type="password" name="mymail_options[<?php echo MYMAIL_MANDRILL_ID; ?>_apikey]" value="<?php echo esc_attr( mymail_option( MYMAIL_MANDRILL_ID . '_apikey' ) ); ?>" class="regular-text" placeholder="xxxxxxxxxxxxxxxxxxxxxx" autocomplete="new-password"></td>
			</tr>
			<tr valign="top">
				<th scope="row">&nbsp;</th>
				<td>
					<?php if ( $verified ) : ?>
					<span style="color:#3AB61B">&#10004;</span> q<?php esc_html_e( 'Your credentials are ok!', 'mymail-mandrill' ); ?>
					<?php else : ?>
					<span style="color:#D54E21">&#10006;</span> <?php esc_html_e( 'Your credentials are WRONG!', 'mymail-mandrill' ); ?>
					<?php endif; ?>
		...
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><?php _e( 'Send Emails with', 'mymail-mandrill' ); ?></th>
				<td>
		...
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><?php _e( 'SMTP Port', 'mymail-mandrill' ); ?></th>
				<td>
				<select name="mymail_options[<?php echo MYMAIL_MANDRILL_ID; ?>_port]">
					...
					<option value="465"<?php selected( mymail_option( MYMAIL_MANDRILL_ID . '_port' ), 465 ); ?>>465 <?php _e( 'with', 'mymail-mandrill' ); ?> SSL</option>
		...
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><?php _e( 'Use subaccount', 'mymail-mandrill' ); ?></th>
				<td>
				<select name="mymail_options[<?php echo MYMAIL_MANDRILL_ID; ?>_subaccount]">
					<option value=""<?php selected( mymail_option( MYMAIL_MANDRILL_ID . '_subaccount' ), 0 ); ?>><?php _e( 'none', 'mymail-mandrill' ); ?></option>
				<?php
				...
				?>
				</select> <span class="description"><?php echo sprintf( __( 'Create new subaccounts on %s', 'mymail-mandrill' ), '<a href="https://mandrillapp.com/subaccounts" class="external">' . __( 'your Mandrill Dashboard', 'mymail-mandrill' ) . '</a>' ); ?></span></td>
			</tr>
		</table>
		<?php endif; ?>
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><?php _e( 'Track in Mandrill', 'mymail-mandrill' ); ?></th>
				<td>
				<select name="mymail_options[<?php echo MYMAIL_MANDRILL_ID; ?>_track]">
					<option value="0"<?php selected( mymail_option( MYMAIL_MANDRILL_ID . '_track' ), 0 ); ?>><?php _e( 'Account defaults', 'mymail-mandrill' ); ?></option>
					<option value="none"<?php selected( mymail_option( MYMAIL_MANDRILL_ID . '_track' ), 'none' ); ?>><?php _e( 'none', 'mymail-mandrill' ); ?></option>
					<option value="opens"<?php selected( mymail_option( MYMAIL_MANDRILL_ID . '_track' ), 'opens' ); ?>><?php _e( 'opens', 'mymail-mandrill' ); ?></option>
					<option value="clicks"<?php selected( mymail_option( MYMAIL_MANDRILL_ID . '_track' ), 'clicks' ); ?>><?php _e( 'clicks', 'mymail-mandrill' ); ?></option>
					<option value="opens,clicks"<?php selected( mymail_option( MYMAIL_MANDRILL_ID . '_track' ), 'opens,clicks' ); ?>><?php _e( 'opens and clicks', 'mymail-mandrill' ); ?></option>
				</select> <span class="description"><?php _e( 'Track opens and clicks in Mandrill as well', 'mymail-mandrill' ); ?></span></td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e( 'Update Limits', 'mymail-mandrill' ); ?></th>
				<td><label><input type="hidden" name="mymail_options[<?php echo MYMAIL_MANDRILL_ID; ?>_autoupdate]" value=""><input type="checkbox" name="mymail_options[<?php echo MYMAIL_MANDRILL_ID; ?>_autoupdate]" value="1" <?php checked( mymail_option( MYMAIL_MANDRILL_ID . '_autoupdate' ), true ); ?>> <?php _e( 'auto update send limits (recommended)', 'mymail-mandrill' ); ?> </label></td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e( 'max emails at once', 'mymail-mandrill' ); ?></th>
				...
				<span class="description"><?php _e( 'define the most highest value for auto calculated send value to prevent server timeouts', 'mymail-mandrill' ); ?></span>
				</td>
			</tr>
		</table>
	...
	public function section_tab_bounce() {
		?>
		<div class="error inline"><p><strong><?php _e( 'Bouncing is handled by Mandrill so all your settings will be ignored', 'mymail-mandrill' ); ?></strong></p></div>
		...
		if ( $options['deliverymethod'] == MYMAIL_MANDRILL_ID ) {
			...
			if ( ( $options[ MYMAIL_MANDRILL_ID . '_username' ] && $options[ MYMAIL_MANDRILL_ID . '_apikey' ] ) ) {
				...
				if ( is_wp_error( $limits ) ) {
					...
					add_settings_error( 'mymail_options', 'mymail_options', __( 'An error occurred:<br>', 'mymail-mandrill' ) . $limits->get_error_message() );
					...
					if ( $limits && $options[ MYMAIL_MANDRILL_ID . '_autoupdate' ] ) {
			...
			if ( $options[ MYMAIL_MANDRILL_ID . '_autoupdate' ] ) {
			...
			if ( function_exists( 'fsockopen' ) && mymail_option( MYMAIL_MANDRILL_ID . '_api' ) == 'smtp' ) {
				...
				if ( is_resource( $conn ) ) {
					...
					add_settings_error( 'mymail_options', 'mymail_options', sprintf( __( 'Not able to use Mandrill via SMTP cause of the blocked port %s! Please try a different port, send with the WEB API or choose a different delivery method!', 'mymail-mandrill' ), $port ) );
	...
	public function update_limits( $limits, $update = true ) {
		...
		($limits['backlog'])
			? mymail_notice( sprintf( __( 'You have %1$s mails in your Backlog! %1$s', 'mymail-mandrill' ), '<strong>' . $limits['backlog'] . '</strong>', '<a href="http://eepurl.com/rvxGP" class="external">' . __( 'What is this?', 'mymail-mandrill' ) . '</a>' ), 'error', true, 'mandrill_backlog' )
			: mymail_remove_notice( 'mandrill_backlog' );
	...
	<div id="message" class="error">
		<p>
		<strong>Mandrill integration for MyMail</strong> requires the <a href="https://evp.to/mymail?utm_campaign=wporg&utm_source=Mandrill+integration+for+MyMail">MyMail Newsletter Plugin</a>, at least version <strong><?php echo MYMAIL_MANDRILL_REQUIRED_VERSION; ?></strong>. Plugin deactivated.
		</p>
	</div>
		<?php
	}
	...
	public function activate() {
		...
		if ( function_exists( 'mymail' ) ) {
			...
			mymail_notice( sprintf( __( 'Change the delivery method on the %s!', 'mymail-mandrill' ), '<a href="options-general.php?page=newsletter-settings&mymail_remove_notice=delivery_method#delivery">Settings Page</a>' ), '', false, 'delivery_method' );
			if ( ! wp_next_scheduled( 'mymail_mandrill_cron' ) ) {
